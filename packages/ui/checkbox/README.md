...
移动端复选组件，React 与小程序独立实现，API 语义一致，仅共享样式 variants。
...
{
  "usingComponents": {
    "sr-checkbox": "@srcube-ui/checkbox/mini",
    "sr-checkbox-group": "@srcube-ui/checkbox/mini/checkbox-group"
  }
}
```
...
| orientation | 排列方向 | `"vertical" \| "horizontal"` | `"vertical"` | 全平台 |
| isBlock | 宽度占满父级 | `boolean` | `false` | 全平台 |
| color | 默认颜色 | `Checkbox['color']` | - | 全平台 |
| size | 默认尺寸 | `Checkbox['size']` | - | 全平台 |
| radius | 默认圆角 | `Checkbox['radius']` | - | 全平台 |
| isDisabled | 禁用态 | `boolean` | `false` | 全平台 |
| isReadOnly | 只读态 | `boolean` | `false` | 全平台 |
| isLineThrough | 选中划线 | `boolean` | `false` | 全平台 |
| className | 根节点 class | React: `string \| ((state) => string)`；Mini: `string` | - | 全平台 |
