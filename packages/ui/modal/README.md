...
移动端弹窗组件，React 与小程序独立实现，API 语义一致，仅共享样式 variants。
...
{
  "usingComponents": {
    "sr-modal": "@srcube-ui/modal/index"
  }
}
```

```xml
<sr-modal isOpen="{{isOpen}}" bind:openchange="handleOpenChange">
...
- 通过子组件组合：`ModalBackdrop` / `ModalContent` / `ModalHeader` / `ModalBody` / `ModalFooter`。
- `ModalBackdrop` 内部会处理遮罩点击并触发关闭（`isDismissable`）。
