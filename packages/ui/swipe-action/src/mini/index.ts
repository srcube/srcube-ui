...
import { type SwipeActionVariants, swipeAction } from '../style';
import type {
...
function resolveWidths(
  data: Pick<
    SwipeActionMiniData,
    'leftActions' | 'rightActions' | 'actionWidth'
  >,
) {
  const actionWidth = resolveActionWidth(data.actionWidth);
...
function buildComputedActions(params: {
  ...
  return normalizeActions(params.actions).map((item) => ({
    ...
    widthStyle: `width: ${params.actionWidth}px;`,
  }));
}
  ...
  computed: {
    ...
    $leftActions(data: SwipeActionMiniData) {
      return buildComputedActions({
        ...
        actionWidth: resolveActionWidth(data.actionWidth),
        color: data.color,
      });
    },
