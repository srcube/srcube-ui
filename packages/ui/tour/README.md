...
```tsx
import { Tour } from '@srcube-ui/tour';
...
const steps = [
  {
    ...
    title: '搜索',
    description: '这里可以快速检索内容',
    placement: 'bottom',
  },
  {
    selector: '#step-filter',
    title: '筛选',
    description: '按条件筛选数据',
    placement: 'bottom',
  },
...
<sr-tour
  ...
  bind:stepchange="handleStepChange"
  bind:skip="handleSkip"
...
Page({
  data: {
    isOpen: false,
    steps: [
      { selector: '#step-search', title: '搜索', description: '这里可以搜索' },
      { selector: '#step-filter', title: '筛选', description: '这里可以筛选' },
    ],
  },
  handleOpenChange(e) {
    this.setData({ isOpen: Boolean(e.detail?.isOpen) });
  },
});
```
