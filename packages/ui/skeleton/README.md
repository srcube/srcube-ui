...
```tsx
import { Skeleton } from '@srcube-ui/skeleton';

export default function Demo() {
  return (
    <Skeleton className="w-full rounded-xl" isLoaded={false}>
...
{
  "usingComponents": {
    "sr-skeleton": "@srcube-ui/skeleton/index"
  }
}
```

```xml
<sr-skeleton className="w-full rounded-xl" isLoaded="{{false}}">
  <view class="h-24 w-full" />
</sr-skeleton>
```
...
- 透传 `div` 原生属性（如 `id/onClick/aria-*`）。
- `children` 作为真实内容，在 `isLoaded=false` 时仅隐藏透明度，用于保持布局尺寸。
