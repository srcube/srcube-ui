...
it("supports dark tone styles", () => {
  ...
  const tabsWrapper = document.querySelector(".bg-primary-950");
  const indicator = document.querySelector('[aria-hidden="true"]');
  const activeTab = screen.getByRole("tab", { name: "Tab A" });

  expect(tabsWrapper).toBeTruthy();
  expect(indicator?.className).toContain("border-primary-700");
  expect(activeTab.className).toContain("text-primary-100");
});
