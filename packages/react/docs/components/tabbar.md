...
<Tabbar
  items=[
    { value: 'home', label: '首页', badge: true },
    { value: 'msg', label: '消息', badge: 12 },
    { value: 'me', label: '我的', badge: '99+' },
  ]
/>
```
...
| label | 标签文案 | React: `ReactNode`；Mini: `string` | `""` |
| icon | 图标内容 | React: `ReactNode`；Mini: `string` | - |
| badge | 右上角角标（`true` 红点；`number/string` 文本角标，如 `99+`） | `boolean \| number \| string` | - |
