...
it("uses default variant and placement in mini props", () => {
  ...
  expect(tabsMiniProps.tone.value).toBe("default");
});
...
it("supports dark tone styles", () => {
  ...
  expect(classNames.tabsWrapper).toContain("bg-primary-950");
  expect(classNames.indicator).toContain("border-primary-700");
});
