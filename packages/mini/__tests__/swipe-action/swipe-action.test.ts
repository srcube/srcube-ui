...
import template from '../../src/components/swipe-action/index.wxml?raw';
...
beforeAll(async () => {
  await import('../../src/components/swipe-action/index');
});
...
it('uses size-based default action width when actionWidth is missing', () => {
  const small = renderSwipeAction({
    defaultOpenDirection: 'right',
    rightActions: [{ key: 'delete', label: 'Delete' }],
    size: 'sm',
  });
  expect(small.data._offset).toBe(-56);
  small.detach();

  const large = renderSwipeAction({
    defaultOpenDirection: 'right',
    rightActions: [{ key: 'delete', label: 'Delete' }],
    size: 'lg',
  });
  expect(large.data._offset).toBe(-72);
  large.detach();
});
