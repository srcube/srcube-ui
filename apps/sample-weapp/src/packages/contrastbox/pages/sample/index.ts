...
function buildSwatch(family: string, step: string, value: string): SwatchItem {
  return {
    ...
    step,
    value,
    textColor: pickReadableText(value),
  };
}

const DISPLAY_STEPS = ['50', '200', '400', '500', '600', '800', '950'] as const;

const SWATCH_GROUPS = Object.entries(TAILWIND_SWATCHES).map(([family, shades]) => ({
  family,
  title: family,
  items: DISPLAY_STEPS.map((step) => buildSwatch(family, step, shades[step])),
}));
...
Page({
  data: {
    ...
    selectedLabel: `${DEFAULT_SELECTED.family}-${DEFAULT_SELECTED.step}`,
  ...
  handleSelectSwatch(e: WechatMiniprogram.TouchEvent) {
    ...
    this.setData({
      ...
      selectedLabel: `${swatch.family}-${swatch.step}`,
